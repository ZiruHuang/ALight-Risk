...
def mutil_model(x_train, y_train, x_test, y_test, output_dir,keyname):
    ...
    for config in model_configs:
        ...
        # 超参数搜索
        if param_grid:
            grid_search = GridSearchCV(model, param_grid=param_grid, scoring='roc_auc', cv=10, n_jobs=13)
        ...
        #cv_prob = cross_val_predict(best_model, x_train_to_use, y_train, cv=10, method='predict_proba')[:, 1]
...
for i in range(1, 2):  # 遍历 subModel_1 到 subModel_7
    trainpath = f'5.MRMD_Result/Lambda/trainset_{i}/'
    outpath = f'6.Model/Lambda/trainset_{i}/'
    for file in os.listdir(trainpath):
        if file.endswith('_reduce.csv'):
            ...
            # 读取测试集并统一维度
            test_file = f'3.iFeature/Lambda/test/Lambda_test_244_{feature_name}.csv'
...
def model_predict(x_test, y_test, model_dir, scaler):
    ...
    for model_file in model_files:
       ...
       # use_scaler = True if model_name in ['SVM',  'KNN'] else False
        if model_name.endswith('SVM') or model_name.endswith('KNN'):
            use_scaler = True
        else:
            use_scaler = False
        x_test_to_use = x_test_scale if use_scaler else x_test
...
for i in range(1, 2):  # 遍历 subModel_1 到 subModel_7
    trainpath = f'5.MRMD_Result/Lambda/trainset_{i}/'
    modelpath = f'6.Model/Lambda/trainset_{i}/'
    outpath = f'7.TrainingScore/Lambda/trainset_{i}/'
    for file in os.listdir(trainpath):
        ...
        if file.endswith('_reduce.csv'):
            ...
            # 读取测试集并统一维度
            test_file = f'3.iFeature/Lambda/subModel_1/Lambda_trainset-1_{feature_name}.csv'
            df_test = pro_test_dimention(test_file, os.path.join(trainpath, file))
            x_test = df_test.iloc[:, 1:].values
            y_test = df_test.iloc[:, 0].values
