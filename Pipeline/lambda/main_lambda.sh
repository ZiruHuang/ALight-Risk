#!/usr/bin/env bash
# ALight-Risk lambda-chain training pipeline
#
...
#   ./main_lambda.sh
#
# The pipeline uses checkpoint files under .checkpoints/lambda so interrupted
...
CHAIN="Lambda"
CHAIN_KEY="lambda"
MRMD_SCRIPT="lambda_mrmd3_submodel_1.sh"
...
for ((round = START_ROUND; round <= END_ROUND; round++)); do
    ...
    if ! is_complete "round_${round}_mrmd"; then
        ...
        mapfile -d '' mrmd_outputs < <(find "$MRMD_WORK_DIR/Results" -maxdepth 1 -mindepth 1 -name 'Lambda_trainset-1*' -print0)
        ((${#mrmd_outputs[@]} > 0)) || die "No Lambda MRMD outputs were produced"
